@testset "Geo interface" begin
    ...
    pt = LibGEOS.Point(LibGEOS._readgeom("POINT EMPTY"))
    @test GeoInterface.coordinates(pt) ≈ Float64[] atol=1e-5
    @test GeoInterface.geotype(pt) == :Point

    mpt = LibGEOS.MultiPoint(LibGEOS._readgeom("MULTIPOINT(0 0, 10 0, 10 10, 11 10)"))
    ...
    ls = LibGEOS.LineString(LibGEOS._readgeom("LINESTRING EMPTY"))
    @test GeoInterface.coordinates(ls) == []
    @test GeoInterface.geotype(ls) == :LineString

    mls = LibGEOS.MultiLineString(LibGEOS._readgeom("MULTILINESTRING ((5 0, 10 0), (0 0, 5 0))"))
    ...
    polygon = LibGEOS.Polygon(LibGEOS._readgeom("POLYGON EMPTY"))
    @test GeoInterface.coordinates(polygon) == [[]]
    @test GeoInterface.geotype(polygon) == :Polygon

    multipolygon = LibGEOS.MultiPolygon(LibGEOS._readgeom("MULTIPOLYGON(((0 0,0 10,10 10,10 0,0 0)))"))
    ...
    geomcollection = LibGEOS.GeometryCollection(LibGEOS._readgeom("GEOMETRYCOLLECTION(MULTIPOINT(0 0, 0 0, 1 1),LINESTRING(1 1, 2 2, 2 2, 0 0),POLYGON((5 5, 0 0, 0 2, 2 2, 5 5)))"))
    collection = GeoInterface.geometries(geomcollection)
    ...
    geomcollection = LibGEOS.GeometryCollection(LibGEOS._readgeom("GEOMETRYCOLLECTION EMPTY"))
    collection = GeoInterface.geometries(geomcollection)
